        ...
        <div class="w-1/4">
            <form wire:submit.prevent="addCategory" class="flex ">
                <div class="flex flex-col">
                    <label for="category">Voeg een categorie toe</label>
    ...
    <div class="grid grid-cols-2 mt-5"> 
        <div>
            <div class="flex flex-col w-2/4" >
                <label for="">Select Recipe</label>
                <select multiple="multiple" data-placeholder="Select a categorie">
                    @foreach ($recipes as $recipe)
                        <option wire:click="selectRecipe({{$recipe->id}})" value="{{$recipe->id}}">{{$recipe->recipe_name}}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="flex flex-col gap-5">
            <div class="grid grid-cols-4">
                <div class="bg-indigo-400 flex flex-col items-center rounded col-span-3">
                        <h3 class="font-bold">Selected Recipe</h3>
                    @if ($selectedRecipe)
                        <h3 class="text-xl">{{$selectedRecipe}}</h3>
                    @else
                        <h3 class="text-xl">Nothing selected</h3>
                    @endif
                </div>
                <div class="col-span-1">
                    <button class="bg-red-400 rounded p-2 ml-2 font-semibold text-whtie" wire:click.prevent="addCategoriesToRecipe">Add Categories</button>
                </div>
            </div>
